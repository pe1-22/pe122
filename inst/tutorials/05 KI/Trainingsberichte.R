...
# ── SCHRITT 1: PubMed-Suche direkt aus R ───────────────
# rentrez nutzt die offizielle NCBI Entrez API (kostenlos, kein Key nötig)
...
# ── SCHRITT 2: Abstracts herunterladen ─────────────────
raw <- entrez_fetch(
  db      = "pubmed",
  id      = suche$ids,
  rettype = "abstract",
...
# ── SCHRITT 3: Abstracts in Dataframe parsen ───────────
# Abstracts sind durch doppelte Leerzeilen getrennt
...
# ── SCHRITT 4: Autoren + Jahr ergänzen (optional) ──────
...
# ── SCHRITT 5: Als CSV speichern (Ausgangsdatei für Übung) ─
